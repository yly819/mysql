...
当您创建帐户使用`INSERT`,有必要使用`FLUSH PRIVILEGES`告诉来服务器,重新加载`grant`表。否则,变更被忽视,直到你重新启动服务器。但是`CREATE USER`时就不用`FLUSH PRIVILEGES`了。

使用`PASSWORD()`函数与插入是加密密码。`CREATE USER`语句你的密码加密,所以`PASSWORD()`是没有必要的。
`
“Y”价值观的账户启用权限。取决于你的MySQL版本,您可能使用不同数量的“Y”值在前两个`insert`语句。`insert`语句`admin`帐户使用更具可读性扩展`insert`语法使用`set`。

在`INSERT`语句中对`dummy`账户,只有`Host,User,`和`Password`列用户表行分配值。没有一个特权列设置明确,所以MySQL赋予他们所有的默认值“N”。这相当于`CREATE USER`。

如果严格SQL模式被激活,所有列,没有默认值必须有一个指定的值。在这种情况下,`insert`语句必须显式地指定值的`ssl_cipher,x509_issuer,`和`x509_subject`列。

建立一个超级用户帐户,有必要只插入一个`user`表行与列的所有特权设置为“Y”。`user`表的特权是全局性的,所以没有条目在任何其他的`grant`表需要。

接下来的例子创建三个账户,并获得特定的数据库。他们的每一个用户名和密码都有初始定义。

创建帐户使用CREATE USER和GRANT,使用以下语句:
...
* 第二个帐户可以访问`expenses`数据库,但只有从主机 `host47.example.com`。　

* 第三个帐户可以访问`customer`数据库,但只有从主机`server.domain`。
...
前三个`INSERT`语句添加`user`表条目,允许用户`custom`从各种主机与给定的密码连接,但是授权没有全局特权(所有权限设置为默认值为“N”)。接下来的三个插入语句添加数据库表,自定义授予特权`bankaccount,expenses,`和`customer`数据库,但只有指定的主机当访问。像往常一样当你直接修改`grant`表,你必须告诉服务器重新加载`FLUSH PRIVILEGES`,特权更改才生效。
...
删除一个账号,用户可以使用`DROP`语句,在 [13.7.1.3,“删除用户语法”][13.07.01.03]有介绍。
...
声明客户端可以对查询问题数量限制(除非它的结果是提供服务查询缓存)。修改数据库或表不利于更新限制。

一个“账户”在这个上下文与`mysql.user`表中的一行。这是一个连接评估值对`User`和`Host`在用户表行,适用于连接。对于例子,一个帐户的`"usera‘@’% .example.com”`对应的一行中用户表,用户和主机的价值观`'usera'@'%.example.com'`,允许从任何主机`user`连接在`example.com`域。在这种情况下,服务器应用资源限制在这行集体所有通过从任何主机连接`usera`在`example.com`域,因为所有这些连接使用相同的帐户。
...
限制类型不需要所有被命名为`with`语句,但这些命名可以存在顺序。为每一个每小时价值限制应该是一个整数代表一个计数每小时。为`max_user_connections`,极限是一个整数代表最大并发连接数,由账户。如果这一上限设置为零,`max_user_connections`系统变量值决定了同步连接的数量。如果`max_user_connections`也是零,是没有限制的账户。

修改现有的限制的账户,使用全局GRANT语句用法(on *.*)。下面的语句更改查询限francis 100:
